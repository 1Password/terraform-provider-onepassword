...
const (
	...
	fieldValueWriteOnceDescription        = "A write-only field value for this item. This value is not stored in the state and is intended for use with ephemeral values. **Note**: Write-only arguments require Terraform 1.11 or later."
	fieldValueWriteOnceVersionDescription = "An integer that must be incremented to trigger an update to the 'field_value_wo' field."
