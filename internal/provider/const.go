...
const (
	...
	rootFieldMapDescription = "A map of all API root-level fields whose `SectionID` is empty, keyed by field label. This includes both standard fields (username, password, etc.) and any custom fields added at the API root level. Fields that appear unsectioned in the 1Password UI may still be stored under the built-in `add more` section and are exposed through `section_map` instead."
