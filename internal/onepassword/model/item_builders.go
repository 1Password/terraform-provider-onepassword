...
func toProviderFieldType(sdkType sdk.ItemFieldType) ItemFieldType {
	switch sdkType {
	case "Text":
		return "STRING"
	case "SshKey":
...
func toProviderCategory(category string) ItemCategory {
	switch category {
	case "SecureNote":
...
func toProviderURLs(websites []sdk.Website) []ItemURL {
...
func toProviderSections(item *sdk.Item, sectionMap map[string]*ItemSection) []*ItemSection {
...
func toProviderFields(item *sdk.Item, sectionMap map[string]*ItemSection) []*ItemField {
	...
	for _, f := range item.Fields {
		field := &ItemField{
			ID:    f.ID,
			Label: f.Title,
			Type:  toProviderFieldType(f.FieldType),
			Value: f.Value,
		}
...
func toProviderFiles(item *sdk.Item) []*ItemFile {
